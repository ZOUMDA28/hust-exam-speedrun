...
description: "华科课程考试速通复习资料生成器：从零基础到 80+ 分。当用户几天内要考试、要求'速通/速成/抱佛脚/零基础'复习、或要求'以后做资料只生成html'时触发。**唯一交付物：单文件交互式 HTML 讲义**（KaTeX 实时公式 + 10 种语义块 + 可折叠答案 + 侧边锚点导航 + 顶部阅读进度 + 知识点标注(.kp) + 分步解答(.sol-step)），按题型套路组织。学习路线图/必背公式/解题套路/例题精讲/作业详解/速记口诀——一份 HTML 自带完整复习链路。支持三种新文件类型：综合精讲、冲刺计划、考前速通。覆盖工作流：调研样卷确定题型 → 写 HTML 讲义 → 本地保存供浏览器打开 → 可选上传飞书作为附件共享。适用于任何一门有样卷和作业答案的课程。"
agent_created: true
---
...
| `.kp` | 紫小标签 | 无（内嵌） | **知识点标签**：紫色小标签，内嵌在解答步骤旁标注所用公式/考点 |
| `.sol-step` | 蓝左边框 | 步骤编号 | **解题步骤**：蓝色左边框，分步呈现解答过程，每步配 .kp 知识点标注 |
| `.formula-block` | 浅蓝背景 | label标签 | **公式块**：带 label 标签的公式展示块，可容纳多个公式 |
| `.checklist` | 勾选框 | ☐/☑ | **勾选清单**：用于冲刺计划/复习清单，可交互勾选 |
| `.tag` + `.tag-must`/`.tag-aim` | 红/橙底 | 文字标签 | **难度/重要性标签**：.tag-must 必考(红)、.tag-aim 目标分(橙)，用于标注考点优先级 |
...
除了按章节/题型的单篇讲义外，还需产出以下三份综合性资料：

#### 1. 综合精讲（`计基_综合_五大题型套路与真题精讲.html`）

- **定位**：考前总复习精华，把整门课压缩成"五大题型 + 真题精讲 + 自测题集"
- **结构**：
  - 考试概况与分值分布表
  - 五大题型套路精讲（每题含：题型特征 → 核心公式 → 解题步骤 → 真题示例）
  - 真题精讲（3~5 套典型真题逐题详解，每题用 .sol-step 分步 + .kp 标注）
  - 自测题集（不少于 20 道，覆盖所有题型，答案用 .collapse 折叠）
  - 必背公式速查表

#### 2. 冲刺计划（`计基_冲刺计划_XX月XX-XX日.html`）

- **定位**：精确到小时的考前倒计时时间表，让学生知道"每小时该干什么"
- **结构**：
  - 倒计时总览表（`.timer-table`，含天数/小时数/任务量）
  - 每日时间表（精确到小时，上午/下午/晚上分段）
  - 资料清单与优先级（`.checklist` 勾选清单，标注重要性 .tag-must/.tag-aim）
  - 刷题优先级排序（真题 > 作业 > 课后题）
  - 每日目标与验收标准

#### 3. 考前速通（`考前速通冲刺指南.html`）

- **定位**：考前最后 1~2 天的"抱佛脚指南"，只看最核心的拿分点
- **结构**：
  - 考试概况（题型/分值/时长/注意事项）
  - Day 1 攻坚计划（地毯式过核心考点）
  - Day 2 冲刺计划（真题模拟 + 查漏补缺）
  - 必背公式清单（一页纸，最核心的 20~30 个公式）
  - 易错点 TOP 20（`.warn` 列表，考场最容易丢分的地方）
  - 考场策略（时间分配 / 答题顺序 / 检查技巧）
...
- **每道例题的解答必须分步骤（.sol-step）**，每步旁标注所用知识点/公式（.kp），做到"一步一知识点"。
- **自测题集不少于 20 道**，覆盖所有题型，难度分层（基础/中等/拔高），标注 .tag-must/.tag-aim。
- **解答用可折叠（.collapse）**，先遮答案，点击展开后显示 .sol-step 分步详解 + .kp 知识点标注。
- 综合精讲、冲刺计划、考前速通三种文件**必须配套产出**，不能只有章节讲义没有总复习资料。
